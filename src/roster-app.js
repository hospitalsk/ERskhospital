...
function renderRosterMatrix() {
  ...
  nurses.forEach(n => {
    ...
    for (let d = 1; d <= days; d++) {
      ...
      const clickAttr = canEdit ? `onclick="openCellEditModal('${n.staffId}', '${dStr}', '${sc}')" class="cursor-pointer hover:bg-sky-100 transition-colors"` : '';
...
async function executeAutoSchedule() {
  ...
  Swal.fire({
    ...
    text: `บันทึกข้อมูลเดือน ${THAI_MONTHS[m]} ${y} รวม ${newSchedules.length} รายการ`
  });
}
